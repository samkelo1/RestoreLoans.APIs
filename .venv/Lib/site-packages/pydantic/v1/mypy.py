...
def from_orm_callback(ctx: MethodContext) -> Type:
    """
    Raise an error if from_attributesis not enabled
    """
    model_type: Instance
    ctx_type = ctx.type
    ...
    if pydantic_metadata is None:
        return ctx.default_return_type
    from_attributes= pydantic_metadata.get('config', {}).get('orm_mode')
    if from_attributesis not True:
